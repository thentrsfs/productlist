		...
		<div className='px-7 py-6 mt-8 xl:mt-[-3.5rem] xl:h-fit bg-white rounded-xl '>
			...
			{cart.map((item) => (
				<ul key={item.id}>
					<li
						key={item.id}
						className='flex justify-between text-4 py-3 '>
						...
						<button
							className='border border-rose-400 text-rose-400 rounded-xl my-[15px] p-[0.2rem] pb-[0.3rem] cursor-pointer hover:border-rose-900 hover:text-rose-900 leading-0 flex items-center justify-center w-[20px] h-[20px] transition-all duration-300'
			...
			<div className={`${cart.length !== 0 ? 'block' : 'hidden'}`}>
				...
				<button
					className='bg-red text-white py-3 rounded-3xl w-full cursor-pointer font-redhat-semibold hover:bg-darkred transition-all duration-300'
